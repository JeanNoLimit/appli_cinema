...
class FilmController {
    ...
    public function formulaireFilm(){
         ...
         // **************************REQUETES POUR L'AFFICHAGE DU FORMULAIRE****************************//
        ...
        $requete_real = $pdo->query('SELECT nom, prenom, id_realisateur
         ...
         // **************************TRAITEMENT DES DONNEES RECEPTIONNEES DANS LE FORMULAIRE****************************//

        if(isset($_POST['submitFilm'])){

            // **************************VERIFICATIONS FICHIER IMAGE****************************//

             // Pour uploader une image, suivre le tuto : https://espritweb.fr/comment-uploader-une-image-en-php/
             // 1. On récupère les données de l'image
            ...
            // 2.On va vérifier l'extension du fichier pour n'accepter que les images
            ...
            if(in_array($extension, $extensions) && $size<=$tMax && $error==0 ) {
                ...
                echo "Mauvaise extension ou image trop importante ou image absente.";
                $file=null;
            }
            ...
            if($titre && $id_genres && $date_sortie && $duree && $id_realisateur){
               if(strlen($titre)>100){
                    $_SESSION['messageAlert']="Le titre ne doit pas dépasser 100 caractères.";
               }elseif(count($id_genres)>3){
                    $_SESSION['messageAlert']="Veuillez rentrer 3 genres au maximum.";
               }elseif( date("Y-m-d")<$date_sortie){
                    $_SESSION['messageAlert']="Veuillez rentrer une date valide.";
               }elseif($duree>1265 || $duree<=0){
                    $_SESSION['messageAlert']="Veuillez rentrer une durée valide.";
               }else{
                    $pdo= Connect::seConnecter();
                    ...
                    $_SESSION['messageSucces']="Film enregistré avec succès.";
               }
            }    
        }
